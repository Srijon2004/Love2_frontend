...
//     const file = e.target.files[0];
//     if (!file) return;
//     setPhotoFile(file); // Store file for submission
...
//       if (photoFile) {
//         formData.append("photo", photoFile); // This matches upload.single("photo") in backend
//       }
...
export default function ProposeForm() {
  ...
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    setPhotoFile(file); // Store file object for submission
    setPreview(URL.createObjectURL(file)); // Instant preview without Base64
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formData = new FormData(); // Required for Cloudinary/Multer
      formData.append("name", form.name);
      formData.append("details", form.details);
      if (photoFile) {
        formData.append("photo", photoFile); // Key must match backend upload.single("photo")
      }
